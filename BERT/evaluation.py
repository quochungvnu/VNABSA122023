...
def get_dataset(data_dir="data", dataset_type="sentihood"):
    ...
    for aspect in semeval_aspects:
        data[aspect] = pd.read_csv(f"{data_dir}/{dataset_type}/BERT-single/{aspect}/test.csv", sep='\t').values.tolist()
    for i in range(len(semeval_aspects)):
        for j in range(len(data[semeval_aspects[i]])):
            original_sentences.append(data[semeval_aspects[i]][j][1])
            auxiliary_sentences.append(data[semeval_aspects[i]][j][2])
            labels.append(data[semeval_aspects[i]][j][3])
...
def get_predictions(predictions_dir, task, dataset_type):
    ...
    if task.endswith("M"):
        ...
        if dataset_type == "sentihood":
            ...
            if task.endswith("B"):
                data = pd.read_csv(f"{predictions_dir}/{dataset_type}/BERT-pair/{task}.csv").values.tolist()
                count_aspect_rows = 0
                current_aspect_scores = []
                for row in data:
                    current_aspect_scores.append(row[2])
                ...
                data = {}
                for aspect in semeval_aspects:
                    data[aspect] = pd.read_csv(f"{predictions_dir}/{dataset_type}/BERT-single/{aspect}.csv").values.tolist()
                for i in range(len(semeval_aspects)):
                    for j in range(len(data[semeval_aspects[i]])):
...
def compute_semeval_PRF(test_labels, predicted_labels):
    ...
    for i in range(num_examples):
        ...
        predicted_aspects = set()
        for j in range(5):
            if test_labels[i * 5 + j] != 3:
                test_aspects.add(j)
            if predicted_labels[i * 5 + j] != 3:
                predicted_aspects.add(j)
        if len(test_aspects) == 0:
            continue
        intersection = test_aspects.intersection(predicted_aspects)
...
def compute_semeval_PRF_cus(test_labels, predicted_labels):
    num_total_intersection = 0
    num_total_test_aspects = 0
    num_total_predicted_aspects = 0
