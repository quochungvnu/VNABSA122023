...
def get_dataset(tokenizer, type_path, args):
    return ABSADataset(tokenizer=tokenizer, data_dir='mydata', data_type=type_path,
                       paradigm='annotation', task='tasd', max_len=128)
...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        dataloader = DataLoader(train_dataset, batch_size=8, drop_last=True, shuffle=True, num_workers=4)
        t_total = (
            (len(dataloader.dataset) // (8 * max(1, 0)))
            // 1
            * float(20)
        )
...
if args.do_train:
    ...
    model.model.save_pretrained('/kaggle/working/checkpoints')
